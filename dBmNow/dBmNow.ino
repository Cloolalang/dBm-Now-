...
String csvPathForSession(uint8_t id) {
  char buf[16];
  snprintf(buf, sizeof(buf), "/log_%u.csv", id);
  return String(buf);
}
...
void setPower(float pwr) {
  float clamped = (pwr < TX_POWER_MIN) ? TX_POWER_MIN : (pwr > TX_POWER_MAX) ? TX_POWER_MAX
                                                                             : pwr;
...
void csvLogStart(uint8_t sessionId, bool quiet) {
  ...
  if (sessionId == 0) {
    if (!quiet) Serial.println(">> CSV: invalid session ID 0.");
    return;
  }
...
void csvLogDump(uint8_t sessionId) {
  if (csvFileLogging) {
    Serial.println(">> Stop CSV log first (f).");
    return;
  }
  uint8_t target = (sessionId > 0) ? sessionId : activeCsvSession;
  if (target == 0) {
    Serial.println(">> No active session. Use dN (e.g. d1) to dump a specific session.");
    return;
  }
  if (!SPIFFS.begin(true)) {
...
void csvLogErase(uint8_t sessionId) {
  ...
  uint8_t target = (sessionId > 0) ? sessionId : (activeCsvSession > 0) ? activeCsvSession
                                                                        : csvSessionId;
  if (target == 0) {
    Serial.println(">> No session to erase. Use E to remove all log files.");
    return;
  }
  csvLogStop(true);
  if (SPIFFS.begin(true)) {
...
void onDataRecv(const esp_now_recv_info_t *info, const uint8_t *incoming, int len) {
  ...
  if (isMaster) {
    ...
    if (!calibrated && referenceRSSI == 1.0f) {
      referenceRSSI = mRSSI;
      calibrated = true;
    }
    ...
    if (len >= (int)sizeof(Payload)) {
      ...
      if (incomingSess == 0) {
        ...
        if (!csvFileLogging || activeCsvSession != incomingSess) {
          ...
          prefs.putUChar("csvSess", csvSessionId);
          prefs.end();  // persist so e works after power cycle
      ...
      if (rxData.oneWayRF != 0) {
        if (!oneWayRFMode) {
          ...
          prefs.begin("probe", false);
          prefs.putBool("oneWay", true);
          prefs.end();  // persist for power cycle
        }
      } else {
        if (oneWayRFMode) {
          oneWayRFMode = false;
    ...
    const char *rfModeStr = (currentRFMode == MODE_STD) ? "STD" : (currentRFMode == MODE_LR_250K) ? "LR 250k"
                                                                                                  : "LR 500k";
...
void printDetailedStatus() {
  ...
  if (isMaster) {
    ...
    {
      float t = getChipTempC();
      if (t > -100.0f) Serial.printf("  CHIP TEMP   : %.1f C\n", t);
      else Serial.println("  CHIP TEMP   : N/A");
    }
    {
      float a = getActualMaxTxPowerDbm();
      if (a > -100.0f && a < currentPower - 1.0f) Serial.printf("  THERMAL     : Throttled (actual %.1f dBm, requested %.1f dBm)\n", a, currentPower);
      else Serial.println("  THERMAL     : OK");
    }
    ...
    {
      float a = getActualMaxTxPowerDbm();
      if (a > -100.0f && a < currentPower - 1.0f) Serial.printf("  THERMAL   : Throttled (actual %.1f dBm)\n", a);
      else Serial.println("  THERMAL   : OK");
    }
...
void loop() {
  ...
  if (!promiscuousMode && !masterPaused && millis() - lastStatusPrint >= 10000) {
  ...
  if (isMaster) {
    if (promiscuousMode) {
        ...
        if (cmd == 'k') {
          ...
          switch (cmd) {
            ...
            case 'l':
              {
                ...
                const char *modeStr = (pendingRFMode == MODE_STD) ? "STD (802.11)" : (pendingRFMode == MODE_LR_250K) ? "LR 250k"
                                                                                                                     : "LR 500k";
            ...
            case '0':
              ...
              prefs.begin("probe", false);
              prefs.putUChar("rfm", currentRFMode);
              prefs.end();
            ...
            case 'w':
              ...
              prefs.begin("probe", false);
              prefs.putBool("oneWayReq", requestOneWayRF);
              prefs.end();  // persist for power cycle
            ...
            case 't':
              {
                float tClamp = (val < TX_POWER_MIN) ? TX_POWER_MIN : (val > TX_POWER_MAX) ? TX_POWER_MAX
                                                                                          : val;
            ...
            case 'r':
              {
                ...
                uint32_t v = (val <= 0) ? minMs : (val < minMs)                ? minMs
                                                : (val > PING_INTERVAL_MAX_MS) ? PING_INTERVAL_MAX_MS
                                                                               : (uint32_t)val;
      ...
      if (!masterPaused && millis() >= nextPingTime) {
        ...
        if (waitingForPong && !plotMode) {
          ...
          if (requestOneWayRF) {
            linkCondition = "1-way mode";
          } else if (lastKnownRSSI > -80.0) {
            linkCondition = "INTERFERENCE";
            interfMinCounter++;
          } else {
            linkCondition = "SIGNAL TOO LOW";
            rangeMinCounter++;
          }
        ...
        struct tm ti;
        localtime_r(&now, &ti);
        myData.nonce = nonceCounter;
        myData.txPower = currentPower;
        myData.targetPower = remoteTargetPower;
        myData.pingInterval = burstDelay;
        myData.hour = ti.tm_hour;
        myData.minute = ti.tm_min;
        myData.second = ti.tm_sec;
        ...
        if (pendingRFMode <= 2) {
          ...
          if (pendingRFModePingsSent >= PENDING_RF_PINGS) {
            ...
            prefs.end();
            applyRFSettings(currentRFMode);
            if (!plotMode) Serial.printf(">> Now on %s\n", (currentRFMode == MODE_STD) ? "STD (802.11)" : (currentRFMode == MODE_LR_250K) ? "LR 250k"
                                                                                                                                          : "LR 500k");
          }
        }
        digitalWrite(ledPin, HIGH);
        ledTimer = millis() + 30;
        currentLEDState = TX_FLASH;
      }
    }
  } else {
    ...
    if (Serial.available() > 0) {
      ...
      if (cmd == 'm') {
        ...
        long mVal = (raw < 0) ? 0 : (raw > (long)MAX_RECORD_TIME_SEC) ? (long)MAX_RECORD_TIME_SEC
                                                                      : raw;
        ...
        switch (cmd) {
          ...
          case 'w':
            ...
            prefs.begin("probe", false);
            prefs.putBool("oneWay", oneWayRFMode);
            prefs.end();  // persist for power cycle
            if (oneWayRFMode) {
              Serial.begin(SERIAL_BAUD_1WAY_RF);
          ...
          case 'H':
            ...
            prefs.begin("probe", false);
            prefs.putBool("huntT", transponderHuntOnTimeout);
            prefs.end();
          ...
          case 'd':
            {
              uint8_t ds = (uint8_t)max(0L, Serial.parseInt());
              csvLogDump(ds);
              break;
            }
          case 'e':
            {
              uint8_t es = (uint8_t)max(0L, Serial.parseInt());
              csvLogErase(es);
              break;
            }
          case 'E':
          ...
          case 'T':
            {
              ...
              if (sub >= '1' && sub <= '9') {
                ...
                prefs.begin("probe", false);
                prefs.putUInt("trxSettle", trxSettleMs);
                prefs.end();
