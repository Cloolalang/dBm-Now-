...
**Field test results:** In low-noise conditions (quiet channel, LR 250 kbps mode), **lowest RSSI measured: −115 dBm**. Link maintained over **400 m** at max TX power (transponder used a directional TEM Horn antenna).

**Urban / noisy environments — noise-limited operation:** In urban or high-interference environments, **receiver noise is often the performance-limiting factor** for ESP-NOW, rather than path loss or TX power. WiFi, Bluetooth, and other 2.4 GHz traffic raise the effective noise floor, so the link can fail at signal levels that would be fine in a quiet channel. Improving the link in these conditions relies on **channel selection** (use promiscuous scan **`P`** to find a quieter channel, then **`n`** to set it), **Long Range 250 kbps** for better sensitivity, and **antenna placement** (e.g. directional antenna, or moving away from obvious interferers). See the notes above on improving low RSSI measurement for related steps.
