...
**Comparison with a low-cost VNA (e.g. TinyVNA / NanoVNA):** This project measures **RSSI of your own ESP-NOW packets** and **path loss between two devices** — it is a **link tester**, not a vector network analyzer. A VNA (e.g. TinyVNA, NanoVNA) measures **S-parameters** (S11, S21): return loss, VSWR, impedance, cable/insertion loss, and antenna match over frequency. It is calibrated for **passive components** (antennas, cables, filters) and gives traceable, swept measurements. The ESP32 probe gives you **link performance** (RSSI, path loss, symmetry) using the same PHY as your application, at very low cost (two dev boards); it does not give S11/S21, impedance, or swept frequency response. A VNA is better for **antenna tuning**, **cable loss**, and **component characterization**; the ESP32 probe is better for **path loss in a real link**, **propagation**, and **link behaviour** with your actual radios. They are **complementary**: use a VNA to characterise antennas and cables; use this probe to characterise your ESP-NOW link and propagation.
